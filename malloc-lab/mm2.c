...
static void *find_fit(size_t size)
{
    ...
    {
        // 할당되지 않은 블록 + 요청한 크기보다 블록의 크기가 크같다면
...
#ifdef BEST_FIT
    // 현재까지의 최적 블록을 가리키는 포인터 
    void *best_bp = NULL;

    // 빈 블록을 만나면 연산한 값 저장할 임시 변수 
    size_t diff;

    // 빈 블록 크기와 요청 크기 사이의 차이를 저장하는 변수 
    // ~: 틸드, 비트 반전 NOT 연산자
    // -> ~0: 모든 0이 1로, 모든 1은 0으로 뒤집힘
    // => min_diff는 맨 처음 1111111... 로 시작함
    // size_t: 부호 없는 정수 =  모든 비트가 1 => 그 자료형이 담을 수 있는 절대적인 최댓값
    // => 운영체제가 몇 비트인지에 따라 메모리 주소 크기와 최댓값이 달라짐
    // => ~0 사용하면 현재 시스템 환경에 맞춰 자동으로 빈틈없이 꽉 찬 최댓값이 만들어짐
    size_t min_diff = ~0;

    // 힙 처음부터 탐색 시작
    // 에필로그 블록을 만날 때까지
    // -> first fit과 동일하게 처음부터 순회 
    for(bp = heap_listp; GET_SIZE(HDRP(bp)) > 0; bp = NEXT_BLKP(bp))
    {
